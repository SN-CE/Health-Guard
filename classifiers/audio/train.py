#!/bin/python3
# train.py - Audio classifier training for TB detection
import os
import torch
import numpy as np
...
def main():
    ...
    if os.path.exists('../../weights/audio_classifier.pt'):
        model.load_state_dict(torch.load('../../weights/audio_classifier.pt', map_location=device))
        print("Loaded previous best model (audio_classifier.pt)")
    else:
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), '../../weights/audio_classifier.pt')
